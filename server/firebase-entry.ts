...
async function buildApp(): Promise<express.Express> {
  // Make secret values visible as standard env vars before any module
  // that reads process.env (e.g. server/db.ts) is loaded.
  if (!process.env.DATABASE_URL) {
    try { process.env.DATABASE_URL = DATABASE_URL.value(); } catch {}
  }
  if (!process.env.NEON_DATABASE_URL) {
    try { process.env.NEON_DATABASE_URL = NEON_DATABASE_URL.value(); } catch {}
  }
  if (!process.env.SESSION_SECRET) {
    try { process.env.SESSION_SECRET = SESSION_SECRET.value(); } catch {}
  }
