...
async function completeImport() {
  try {
    console.log('🚀 استكمال عملية استيراد بيانات التسلسل الهرمي...');
    ...
    for (const brandData of hierarchyData) {
      ...
      if (manufacturer.length === 0) {
        ...
        console.log(`✓ الشركة موجودة: ${brandData.brand_ar}`);
      ...
      // فاصل زمني صغير لتجنب الحمل الثقيل على قاعدة البيانات
      if (brandCount % 5 === 0) {
    ...
    console.log('\n✅ اكتملت عملية الاستيراد الكاملة بنجاح!');
...
completeImport()
  .then(() => {
    console.log('🎉 تم انهاء العملية الكاملة بنجاح');
    process.exit(0);
  })
  .catch((error) => {
